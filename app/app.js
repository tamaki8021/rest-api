...
app.get("/api/v1/users/:id/following", (req, res) => {
  ...
  db.all(`SELECT * FROM following LIFT JOIN users ON following.followed_id = users.id WHERE following_id = ${id}`, (err, rows) => {
    if (!rows) {
...
app.post("/api/v1/users", async (req, res) => {
  if (!req.body.name || req.body.name === "") {
    res.status(400).send({ error: "ユーザー名が指定荒れていません" });
  } else {
    // connect database
    const db = new sqlite3.Database(dbPath);
  ...
  if (!req.body.name || req.body.name === "") {
    ...
    db.get(`SELECT * FROM users WHERE id = ${id}`, async (err, row) => {
      if (!row) {
        ...
        const profile = req.body.profile ? req.body.body.profile : row.profile;
        const dateOfBirth = req.body.date_of_birth
